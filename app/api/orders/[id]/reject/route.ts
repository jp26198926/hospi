...
import { rejectOrderSchema } from "@/modules/orders/validation";
...
export const POST = executeRoute(async (req) => {
  ...
  const body = rejectOrderSchema.parse(await req.json());
  const meta = getRequestMeta(req);
  const data = await orderService.rejectOrder(id, body.reason, session.user.id, meta);
