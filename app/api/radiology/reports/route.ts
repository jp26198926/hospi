...
import { listReportsQuerySchema, createReportSchema } from "@/modules/radiology/validation";
...
export const GET = executeRoute(async (req) => {
  ...
  const data = await radService.listReports(query);
...
export const POST = executeRoute(async (req) => {
  ...
  const data = await radService.createReport(body, session.user.id, meta);
